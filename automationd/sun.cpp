...
static time_t unix_time_from_jd(double jd)
{
        return 86400.0*(jd - 2440587.5);
}

/* Julian day from unix time */
static double jd_from_unix_time(time_t t)
{
...
static double elevation_from_hour_angle(double lat, double decl, double ha)
{
...
static double time_of_solar_elevation(double t, double t_noon,
...
double solar_elevation(time_t date, double lat, double lon)
{
...
double Sun::get_elevation() {
  time_t now = time(0);
  return solar_elevation(now, latitude_, longitude_);
}
